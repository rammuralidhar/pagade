...
public class Board {
    ...
    public Board() {
        ...
        pawnModel = gloader.loadModel(Gdx.files.internal("meshes/pawn.g3db"));
        bishopModel = gloader.loadModel(Gdx.files.internal("meshes/pawn.g3db"));

        knightWhiteModel = gloader.loadModel(Gdx.files.internal("meshes/knight.g3db"));
        knightBlackModel = gloader.loadModel(Gdx.files.internal("meshes/knight-black.g3db"));
		//knightModel.nodes.get(0).rotation.set(Y_AXIS, 180);

        rookModel = gloader.loadModel(Gdx.files.internal("meshes/rook.g3db"));
        queenModel = gloader.loadModel(Gdx.files.internal("meshes/queen.g3db"));
        kingModel = gloader.loadModel(Gdx.files.internal("meshes/king.g3db"));
    ...
    public Cube getCube(String coord) {
        Cube c = null;
        for (Cube cube : cubes) {
            if (cube.getCoordinate().equals(coord)) {
                c = cube;
            }
        }
        return c;
    ...
    public boolean isSamePosition(Vector3 v1, Vector3 v2) {
        ...
        if (NumberUtils.floatToIntBits(v1.x) != NumberUtils.floatToIntBits(v2.x)) {
            return false;
        }
        if (NumberUtils.floatToIntBits(v1.z) != NumberUtils.floatToIntBits(v2.z)) {
            return false;
        }
        return true;
    }
